...
export default class Timer {
  ...
  private static parseDurationFormat(durationString: string): number {
    ...
    if (!hasMatch) {
      throw new Error("Invalid timer format. Use: 90s, 10m, 1h30m, or 12:00");
    }
  ...
  getRemainingTime(): number {
    ...
    if (this.isPaused) return Math.max(0, Math.ceil((this.endTime - this.pausedTime) / 1000));
  ...
  getFormattedTime(remaining?: number): string {
    const seconds = remaining !== undefined ? remaining : this.getRemainingTime();
