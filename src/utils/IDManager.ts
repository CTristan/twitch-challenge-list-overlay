...
export default class IDManager {
    ...
    private loadFromStorage(): void {
        // Load counter
        const counterResult = StorageManager.load(IDManager.STORAGE_KEY, 1);
        if (counterResult.success && typeof counterResult.data === "number") {
            this.counter = counterResult.data;
        } else {
            this.counter = 1;
        }

        // Load mappings
        const mappingResult = StorageManager.load(IDManager.ID_MAP_KEY, {
            longToShort: {},
            shortToLong: {},
        });

        if (mappingResult.success && mappingResult.data) {
            const mappingData = mappingResult.data as any;
            this.idMap = new Map(Object.entries(mappingData.longToShort || {}));
            this.reverseMap = new Map(
                Object.entries(mappingData.shortToLong || {})
            );
        } else {
            this.idMap.clear();
            this.reverseMap.clear();
    ...
    private saveCounter(): void {
        const result = StorageManager.save(IDManager.STORAGE_KEY, this.counter);
        if (!result.success) {
            console.error("Error saving ID counter to storage:", result.error);
        }
    }
    ...
    private saveMapping(): void {
        ...
        const result = StorageManager.save(IDManager.ID_MAP_KEY, mappingData);
        if (!result.success) {
            console.error("Error saving ID mappings to storage:", result.error);
        }
    }
}
