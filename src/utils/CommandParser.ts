...
/**
 * @class CommandParser
 * Parses robust command syntax with key=value parameters, flexible ordering,
...
export default class CommandParser {
  ...
  static parseCommand(input: string): ParsedCommand {
    ...
    try {
      ...
      if (parts.parameterString) {
        result.parameters = CommandParser.parseParameters(parts.parameterString);
        
        // Resolve aliases
        result.parameters = CommandParser.resolveAliases(result.parameters);
        
        // Validate parameters
        const validation = CommandParser.validateParameters(result.parameters, result.command);
  ...
  private static splitCommand(input: string): {
    ...
    if (command === 'add' || command === 'list' || command === 'help') {
      // Simple commands: "add title=..." or "list"
      return {
        command,
        parameterString: parts.slice(1).join(' ')
      };
    } else if (command === '+' || command === '-' || command === 'set' || 
               command === 'done' || command === 'fail' || command === 'del' || 
               command === 'show' || command === 'edit') {
   ...
   */
  private static parseParameters(paramString: string): ParsedCommandParameters {
   ...
   */
  private static resolveAliases(parameters: ParsedCommandParameters): ParsedCommandParameters {
   ...
   */
  private static validateParameters(parameters: ParsedCommandParameters, command: string): ValidationResult {
    const errors: string[] = [];
    ...
    for (const key of Object.keys(parameters)) {
      if (!CommandParser.VALID_PARAMETERS.has(key)) {
        errors.push(`Unknown parameter '${key}'. Valid parameters: ${Array.from(CommandParser.VALID_PARAMETERS).join(', ')}`);
      }
    }
    ...
    if (parameters.timer !== undefined) {
      try {
        ...
        errors.push(`Timer format invalid: ${error instanceof Error ? error.message : String(error)}`);
      }
    }

    // Command-specific validation
    if (command === 'add' && !parameters.title) {
   ...
   * @returns Parsed parameters
   */
  static parsePositionalCommand(input: string): { index?: number; value?: string; values?: string[] } {
    const trimmed = input.trim();
    if (!trimmed) return {};
