...
export default class AdminPanel {
  ...
  private setupBasicControls(): void {
    ...
    if (clearButton) {
      clearButton.addEventListener("click", () => this.clearLocalStorage());
    }
  }
  ...
  private populateConfigurationForm(): void {
    ...
    this.setInputValue("twitch-username", config.auth?.twitch_username || "");
   ...
   * @returns {void}
   */
  private populateColorConfiguration(backgroundColors: string[], textColors: string[] = []): void {
    const colorConfig = this.convertColorsToUI(backgroundColors, textColors);
    const tiers = ["primary", "secondary", "tertiary"] as const;

    tiers.forEach((tier) => {
      const checkbox = document.getElementById(`${tier}-color-enabled`) as HTMLInputElement;
      const bgColorInput = document.getElementById(`${tier}-bg-color`) as HTMLInputElement;
      const textColorInput = document.getElementById(`${tier}-text-color`) as HTMLInputElement;
  ...
  private setupColorTierEventListeners(): void {
    ...
    colorTiers.forEach((tier) => {
      const checkbox = document.getElementById(`${tier}-color-enabled`) as HTMLInputElement;
      const pickersContainer = document.getElementById(`${tier}-color-pickers`);
      const section = document.getElementById(`${tier}-color-section`);
      const bgColorInput = document.getElementById(`${tier}-bg-color`) as HTMLInputElement;
      const textColorInput = document.getElementById(`${tier}-text-color`) as HTMLInputElement;

      if (checkbox && pickersContainer && section && bgColorInput && textColorInput) {
        // Initial state setup
   ...
   */
  private convertColorsToUI(backgroundColors: string[], textColors: string[] = []): ColorConfigurationUI {
    const defaultConfig: ColorConfigurationUI = {
      primary: { enabled: false, backgroundColor: "#ff0000", textColor: "#ffffff" },
      secondary: { enabled: false, backgroundColor: "#00ff00", textColor: "#ffffff" },
      tertiary: { enabled: false, backgroundColor: "#0000ff", textColor: "#ffffff" }
    };
    ...
    backgroundColors.forEach((backgroundColor, index) => {
      if (index < tiers.length) {
        ...
        defaultConfig[tier].backgroundColor = backgroundColor.trim();
  ...
  private getCurrentColorConfigFromUI(): ColorConfigurationUI {
    const config: ColorConfigurationUI = {
      primary: {
        enabled: (document.getElementById("primary-color-enabled") as HTMLInputElement)?.checked || false,
        backgroundColor: (document.getElementById("primary-bg-color") as HTMLInputElement)?.value || "#ff0000",
        textColor: (document.getElementById("primary-text-color") as HTMLInputElement)?.value || "#ffffff"
      },
      secondary: {
        enabled: (document.getElementById("secondary-color-enabled") as HTMLInputElement)?.checked || false,
        backgroundColor: (document.getElementById("secondary-bg-color") as HTMLInputElement)?.value || "#00ff00",
        textColor: (document.getElementById("secondary-text-color") as HTMLInputElement)?.value || "#ffffff"
      },
      tertiary: {
        enabled: (document.getElementById("tertiary-color-enabled") as HTMLInputElement)?.checked || false,
        backgroundColor: (document.getElementById("tertiary-bg-color") as HTMLInputElement)?.value || "#0000ff",
        textColor: (document.getElementById("tertiary-text-color") as HTMLInputElement)?.value || "#ffffff"
      }
    };
  ...
  private saveConfiguration(): void {
    try {
      ...
      const maxChallenges = parseInt(this.getInputValue("max-challenges"), 10);
      ...
      const challengeRowTextColors = this.convertUIToTextColors(colorConfig);

      // Update configuration
      const authSuccess = this.#configManager.set("auth", authConfig);
      const maxChallengesSuccess = this.#configManager.set("maxChallenges", maxChallenges);
      const colorsSuccess = this.#configManager.set("challengeRowColors", challengeRowColors);
      const textColorsSuccess = this.#configManager.set("challengeRowTextColors", challengeRowTextColors);

      // Check if all configuration updates were successful
      if (authSuccess && maxChallengesSuccess && colorsSuccess && textColorsSuccess) {
        // Visual feedback
        ...
        this.showFeedback("save-config-btn", "Partial Save Error!", "#dc3545");
      }
    } catch (error) {
  ...
  private resetConfiguration(): void {
    try {
      ...
      if (resetSuccess) {
        ...
        this.showFeedback("reset-config-btn", "Reset Failed!", "#dc3545");
      }
    } catch (error) {
  ...
  private importFromFile(fileInput: HTMLInputElement): void {
    ...
    if (!file) {
      this.showFeedback("import-config-btn", "No file selected!", "#dc3545");
      return;
    }
    ...
    reader.onerror = () => {
      this.showFeedback("import-config-btn", "Error reading file!", "#dc3545");
    };
  ...
  private processImportedConfiguration(
    ...
    try {
      ...
      if (!validationResult.isValid) {
        this.showFeedback(buttonId, validationResult.errorMessage, "#dc3545");
        return;
      }
   ...
   * @returns {void}
   */
  private showFeedback(buttonId: string, message: string, color: string): void {
