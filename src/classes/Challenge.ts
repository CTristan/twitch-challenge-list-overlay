...
export default class Challenge {
  ...
  constructor(
    ...
    const { description = "", amount = 1, timer, isLegacy = false } = options;
    ...
    if (isLegacy || (Object.keys(options).length === 0)) {
      ...
      this.description = this.validateDescriptionString(titleOrDescription);
      this.title = this.description; // Use description as title for backward compatibility
    } else {
      // New format: separate title and description
      this.title = this.validateTitle(titleOrDescription);
      // Allow empty descriptions for title-only challenges
      this.description = description ? this.validateDescriptionString(description) : "";
    }
  ...
  validateTitle(title: string): string {
    if (typeof title !== "string") {
      throw new Error("Challenge title must be of type string");
    }
    title = title.trim();
    if (title.length === 0) {
    ...
    if (title.length > 100) {
      throw new Error("Challenge title too long (max 100 characters)");
    }
    return title;
  }
  ...
  validateAmount(amount: number): number {
    if (typeof amount !== "number" || isNaN(amount)) {
      throw new Error("Challenge amount must be a number");
    }
    if (amount < 1) {
  ...
  validateDescriptionString(description: string): string {
    if (typeof description !== "string") {
      throw new Error("Challenge description must be of type string");
    }
    description = description.trim();
    if (description.length === 0) {
      throw new Error("Challenge description invalid");
    }
    return description;
  }
  ...
  setDescription(description: string): void {
    ...
    if (description.length > 200) {
      throw new Error("Challenge description too long (max 200 characters)");
    }
  ...
  setTimer(timer: string | number): void {
    if (typeof timer === "string") {
      ...
      throw new Error("Timer must be a duration string or number of seconds");
    }
  }
  ...
  static fromSerializedData(data: any): Challenge {
    ...
    if (typeof data === "object" && data.description && data.completionStatus !== undefined) {
      const challenge = new Challenge(data.description, { isLegacy: true });
