...
-   **Auto-save methods**: `addChallengeObjects()` and state-transition helpers trigger saves and cross-window notifications
-   **Manual save methods**: `saveToLocalStorage()` (required after challenge setters)
-   **State management**: Status-driven (in-progress/completed/failed) with counter updates and timer control
...
-   **Properties**: title, description, amount, status, timer
-   **Validation**: Input validation in constructor and setters
-   **Setters**: `setTitle()`, `setDescription()`, `setAmount()`, `setTimer()`, `setStatus()`
-   **State API**:
    -   `getStatus()`/`setStatus()` using `ChallengeStatus` enum (IN_PROGRESS, COMPLETED, FAILED)
    -   Convenience checks: `isComplete()`, `isFailed()`
-   **Timer behavior**:
    -   On `COMPLETED` or `FAILED`, the timer stops
    -   When reverted to `IN_PROGRESS`, timer can be restarted via UI actions as needed
-   **No auto-save**: Must call `ChallengeList.saveToLocalStorage()` after direct setters (non-list ops)
