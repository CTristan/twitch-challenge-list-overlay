...
describe("Challenge Row Colors", () => {
  ...
  describe("Two colors configured", () => {
    it("should alternate between two colors", () => {
      ...
      const challengeElements = document.querySelectorAll(".challenge");
      expect((challengeElements[0] as HTMLElement).style.backgroundColor).toBe(
        "rgb(255, 0, 0)"
      ); // red
      expect((challengeElements[1] as HTMLElement).style.backgroundColor).toBe(
        "rgb(0, 255, 0)"
      ); // green
      expect((challengeElements[2] as HTMLElement).style.backgroundColor).toBe(
        "rgb(255, 0, 0)"
      ); // red
      expect((challengeElements[3] as HTMLElement).style.backgroundColor).toBe(
        "rgb(0, 255, 0)"
      ); // green
    });
  });
  ...
  describe("Three colors configured", () => {
    it("should cycle through three colors", () => {
      ...
      expect((challengeElements[4] as HTMLElement).style.backgroundColor).toBe(
        "rgb(0, 255, 0)"
      ); // green
      expect((challengeElements[5] as HTMLElement).style.backgroundColor).toBe(
        "rgb(0, 0, 255)"
      ); // blue
    });
  });
  ...
  describe("Dynamic challenge addition", () => {
    it("should apply correct colors when adding challenges dynamically", () => {
      ...
      const newChallenge = app.challengeList.addChallenges(["Challenge 3"])[0];
...
describe("Challenge Checkbox Color Inheritance", () => {
  ...
  describe("No text colors configured", () => {
    it("should not apply custom checkbox colors when challengeRowTextColors is empty", () => {
      ...
      const checkboxElements = document.querySelectorAll(".challenge-checkbox");
      checkboxElements.forEach((element) => {
        const checkbox = element as HTMLElement;
        expect(checkbox.style.getPropertyValue('--challenge-checkbox-border-color')).toBe("");
        expect(checkbox.style.getPropertyValue('--challenge-checkbox-checked-border-color')).toBe("");
        expect(checkbox.style.getPropertyValue('--challenge-checkbox-checkmark-color')).toBe("");
      });
    });
  });
  ...
  describe("Text colors configured", () => {
    it("should apply checkbox colors to match text colors", () => {
      ...
      expect(firstCheckbox.style.getPropertyValue('--challenge-checkbox-border-color')).toBe("#ff0000");
      expect(firstCheckbox.style.getPropertyValue('--challenge-checkbox-checked-border-color')).toBe("#ff0000");
      expect(firstCheckbox.style.getPropertyValue('--challenge-checkbox-checkmark-color')).toBe("#ff0000");

      // Second checkbox should have green colors
      const secondCheckbox = checkboxElements[1] as HTMLElement;
      expect(secondCheckbox.style.getPropertyValue('--challenge-checkbox-border-color')).toBe("#00ff00");
      expect(secondCheckbox.style.getPropertyValue('--challenge-checkbox-checked-border-color')).toBe("#00ff00");
      expect(secondCheckbox.style.getPropertyValue('--challenge-checkbox-checkmark-color')).toBe("#00ff00");
    });
    ...
    it("should apply checkbox colors when adding challenges dynamically", () => {
      ...
      expect(thirdCheckbox.style.getPropertyValue('--challenge-checkbox-border-color')).toBe("#ff0000");
      expect(thirdCheckbox.style.getPropertyValue('--challenge-checkbox-checked-border-color')).toBe("#ff0000");
      expect(thirdCheckbox.style.getPropertyValue('--challenge-checkbox-checkmark-color')).toBe("#ff0000");
    });

    it("should apply checkbox colors to both primary and secondary containers", () => {
      configManager.set("challengeRowTextColors", ["#ff0000"]);

      const challenge = app.challengeList.addChallenges(["Test challenge"])[0];
      ...
      checkboxElements.forEach((element) => {
        const checkbox = element as HTMLElement;
        expect(checkbox.style.getPropertyValue('--challenge-checkbox-border-color')).toBe("#ff0000");
        expect(checkbox.style.getPropertyValue('--challenge-checkbox-checked-border-color')).toBe("#ff0000");
        expect(checkbox.style.getPropertyValue('--challenge-checkbox-checkmark-color')).toBe("#ff0000");
      });
    });
    ...
    it("should maintain checkbox colors when challenge is completed", () => {
      ...
      const challenge = app.challengeList.addChallenges(["Complete me"])[0];
      app.renderChallengeList();
      ...
      const checkboxElements = document.querySelectorAll(".challenge-checkbox.checked");
      expect(checkboxElements).toHaveLength(2); // Primary and secondary containers
