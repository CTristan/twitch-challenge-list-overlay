...
describe("CommandParser", () => {
  describe("parseCommand", () => {
    it("should parse simple add command with title", () => {
      const result = CommandParser.parseCommand('add title="Kick zombies"');
    ...
    it("should parse add command with multiple parameters", () => {
      const result = CommandParser.parseCommand('add title="Kick zombies" desc="Off the roof" amount=30 timer=10m');
    ...
    it("should parse command with parameter aliases", () => {
      const result = CommandParser.parseCommand('add t="Test" d="Description" a=5 tm=1h');
    ...
    it("should parse edit command with target ID and parameters", () => {
      const result = CommandParser.parseCommand('edit A7 title="New title" amount=50');
    ...
    it("should handle quoted strings with spaces", () => {
      const result = CommandParser.parseCommand('add title="Kick 30 zombies off the roof" desc="Use only melee weapons"');
      
      expect(result.parameters.title).toBe('"Kick 30 zombies off the roof"');
      expect(result.parameters.desc).toBe('"Use only melee weapons"');
      expect(result.isValid).toBe(true);
    });

    it("should handle single quotes", () => {
      const result = CommandParser.parseCommand("add title='Single quoted title' desc='Single quoted desc'");
    ...
    it("should validate required parameters for add command", () => {
      const result = CommandParser.parseCommand('add desc="No title provided"');
      
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain("Title is required for add command");
    });

    it("should validate unknown parameters", () => {
      const result = CommandParser.parseCommand('add title="Test" unknown=value');
      
      expect(result.isValid).toBe(false);
      expect(result.errors.some(e => e.includes("Unknown parameter 'unknown'"))).toBe(true);
    });

    it("should validate amount parameter", () => {
      const result = CommandParser.parseCommand('add title="Test" amount=invalid');
    ...
    it("should validate timer format", () => {
      const result = CommandParser.parseCommand('add title="Test" timer=invalid');
      
      expect(result.isValid).toBe(false);
      expect(result.errors.some(e => e.includes("Timer format invalid"))).toBe(true);
    });
  ...
  describe("unquoteString", () => {
    it("should remove double quotes", () => {
      expect(CommandParser.unquoteString('"Hello World"')).toBe("Hello World");
    });
  ...
  describe("parsePositionalCommand", () => {
    ...
    it("should parse index with value", () => {
      const result = CommandParser.parsePositionalCommand("1 new description");
    ...
    it("should handle single value", () => {
      const result = CommandParser.parsePositionalCommand("description only");
